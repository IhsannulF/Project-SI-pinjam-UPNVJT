...
@section('content')
    ...
    <!-- KOTAK STATUS PENGAJUAN TERAKHIR / BANNER UTAMA -->
    @if($pengajuan_terakhir)
        ...
        <div class="bg-gradient-to-r {{ $bgBanner }} border border-l-4 rounded-2xl md:rounded-3xl p-5 md:p-6 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-5 md:gap-6 relative overflow-hidden transition-all mb-6 md:mb-8">
            ...
            <div class="relative z-10 shrink-0 w-full md:w-auto mt-2 md:mt-0 flex gap-3">
                <a href="{{ route('mahasiswa.riwayat') }}" class="flex justify-center items-center gap-2 bg-[#1a1d24] hover:bg-gray-800 text-white border border-gray-600 px-5 md:px-6 py-2.5 md:py-3 rounded-xl font-semibold transition-all shadow-sm text-sm md:text-base w-full md:w-auto">
                    Lihat Detail
                </a>
            </div>
        </div>
    @else
        <div class="bg-gradient-to-r from-sipdark to-sipbg border border-sipborder border-l-4 border-l-sipblue rounded-2xl md:rounded-3xl p-5 md:p-6 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-5 md:gap-6 relative overflow-hidden mb-6 md:mb-8">
            <div class="absolute -right-5 -bottom-10 text-sipblue/5 pointer-events-none hidden md:block">
                <i class="fas fa-calendar-check text-9xl"></i>
            </div>
            <div class="relative z-10 flex items-center gap-4 md:gap-6">
                <div class="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-sipblue/10 flex items-center justify-center shrink-0">
                    <i class="fas fa-info-circle text-2xl md:text-3xl text-sipblue"></i>
                </div>
                <div>
                    <h5 class="text-base md:text-xl font-bold text-white mb-0.5 md:mb-1">Mulai Peminjaman Baru</h5>
                    <p class="text-xs md:text-sm text-siptext font-medium">Anda belum memiliki pengajuan aktif atau riwayat peminjaman saat ini.</p>
                </div>
            </div>
            <div class="relative z-10 shrink-0 w-full md:w-auto mt-2 md:mt-0">
                <a href="{{ route('mahasiswa.cari_fasilitas') }}" class="flex justify-center items-center gap-2 w-full md:w-auto bg-sipblue hover:bg-sipbluehover text-white px-5 md:px-6 py-2.5 md:py-3 rounded-xl font-semibold transition-all shadow-lg shadow-sipblue/30 active:scale-[0.98] text-sm md:text-base">
    ...
    <!-- GRID BAWAH (STATISTIK DAN RIWAYAT) -->
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 md:gap-6">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-1 gap-3 md:gap-4">
            ...
            <div class="bg-sipdark border border-sipborder rounded-xl md:rounded-2xl p-4 md:p-5 flex items-center justify-between shadow-lg">
                <div>
                    ...
                    <h3 class="text-xl md:text-2xl font-bold text-yellow-500">{{ $stat_pending }}</h3>
                </div>
            ...
            <div class="bg-sipdark border border-sipborder rounded-xl md:rounded-2xl p-4 md:p-5 flex items-center justify-between shadow-lg">
                <div>
                    ...
                    <h3 class="text-xl md:text-2xl font-bold text-[#00AE1C]">{{ $stat_disetujui }}</h3>
                </div>
            ...
            <div class="bg-sipdark border border-sipborder rounded-xl md:rounded-2xl p-4 md:p-5 flex items-center justify-between shadow-lg">
                <div>
                    <p class="text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-widest mb-0.5 md:mb-1">Ditolak</p>
                    <h3 class="text-xl md:text-2xl font-bold text-sipred">{{ $stat_ditolak }}</h3>
                </div>
                <div class="w-10 h-10 md:w-12 md:h-12 rounded-full bg-sipred/10 flex items-center justify-center text-sipred text-lg md:text-xl border border-sipred/20 shrink-0">
                    <i class="fas fa-ban"></i>
                </div>
            </div>
            
            <div class="bg-[#15181f] border border-gray-700 rounded-xl md:rounded-2xl p-4 md:p-5 flex items-center justify-between shadow-sm hover:border-gray-500 transition-colors">
                <div>
        ...
        <div class="lg:col-span-2 bg-sipdark border border-sipborder rounded-2xl md:rounded-3xl p-5 md:p-6 shadow-lg flex flex-col">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('mahasiswa.riwayat') }}" class="text-[10px] md:text-xs font-bold text-sipblue hover:text-white transition-colors">Lihat Semua &rarr;</a>
            </div>
                            ...
                            <tr class="border-b border-gray-700/50 last:border-0 hover:bg-[#15181f] transition-colors">
                                <td class="py-3 pr-4">
                                    ...
                                    <div class="text-[10px] md:text-[11px] text-gray-400 mt-1 flex items-center gap-1.5">
                                        <i class="far fa-calendar-alt"></i> {{ \Carbon\Carbon::parse($item->tanggal_pinjam)->format('d M Y') }}
                                    </div>
                                </td>
                                <td class="py-3 text-right whitespace-nowrap">
                                    @if($item->status == 'pending' || $item->status == 'menunggu')
                                        <span class="bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 px-2 py-1 rounded text-[9px] md:text-[10px] font-bold uppercase tracking-wider">Pending</span>
