    ...
    @if($pengajuan_terakhir)
        ...
        <div class="bg-gradient-to-r {{ $bgBanner }} border border-l-4 rounded-2xl p-6 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden transition-all">
            ...
            <div class="relative z-10 shrink-0 w-full md:w-auto mt-4 md:mt-0 flex gap-3">
                <a href="{{ route('eksternal.riwayat') }}" class="flex justify-center items-center gap-2 bg-[#1a1d24] hover:bg-gray-800 text-white border border-gray-600 px-6 py-3 rounded-xl font-semibold transition-all shadow-sm">
                    Lihat Detail
                </a>
            </div>
        </div>
    @else
        <div class="bg-gradient-to-r from-sipdark to-sipbg border border-sipborder border-l-4 border-l-[#00AE1C] rounded-2xl p-6 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
            <div class="absolute -right-5 -bottom-10 text-[#00AE1C]/5 pointer-events-none">
                <i class="fas fa-handshake text-9xl"></i>
            </div>
            <div class="relative z-10 flex items-center gap-6">
                <div class="w-16 h-16 rounded-2xl bg-[#00AE1C]/10 flex items-center justify-center shrink-0">
                    <i class="fas fa-file-contract text-3xl text-[#00AE1C]"></i>
                </div>
                <div>
                    <h5 class="text-xl font-bold text-white mb-1">Mulai Reservasi Fasilitas</h5>
                    <p class="text-sm text-siptext font-medium">Anda belum memiliki pengajuan aktif. Siapkan dokumen MoU dan Bukti Pembayaran.</p>
                </div>
            </div>
            <div class="relative z-10 shrink-0 w-full md:w-auto mt-4 md:mt-0">
                <a href="{{ route('eksternal.reservasi') }}" class="flex justify-center items-center gap-2 w-full md:w-auto bg-[#00AE1C] hover:bg-[#009017] text-white px-6 py-3 rounded-xl font-semibold transition-all shadow-lg shadow-[#00AE1C]/30 active:scale-[0.98]">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="space-y-4">
            <div class="bg-sipdark border border-sipborder rounded-2xl p-5 flex items-center justify-between shadow-lg">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-yellow-500">{{ $stat_pending ?? 0 }}</h3>
                </div>
            ...
            <div class="bg-sipdark border border-sipborder rounded-2xl p-5 flex items-center justify-between shadow-lg">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-[#00AE1C]">{{ $stat_disetujui ?? 0 }}</h3>
                </div>
        ...
        <div class="lg:col-span-2 bg-sipdark border border-sipborder rounded-2xl p-6 shadow-lg flex flex-col">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('eksternal.riwayat') }}" class="text-xs font-bold text-sipblue hover:text-white transition-colors">Lihat Semua &rarr;</a>
            </div>
                        ...
                        @forelse($riwayat_singkat ?? [] as $item)
                            <tr class="border-b border-gray-700/50 last:border-0 hover:bg-[#15181f] transition-colors">
                                <td class="py-3 pr-4">
                                    ...
                                    <div class="text-[11px] text-gray-400 mt-0.5"><i class="far fa-calendar-alt mr-1"></i> {{ \Carbon\Carbon::parse($item->tanggal_mulai ?? $item->tanggal_pinjam)->format('d M Y') }}</div>
                                </td>
                                <td class="py-3 text-right">
                                    @if($item->status == 'pending' || $item->status == 'menunggu')
                                        <span class="bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider">Menunggu</span>
                                    ...
                                    @elseif($item->status == 'dibatalkan')
                                        <span class="bg-gray-600/10 text-gray-400 border border-gray-600/20 px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider">Dibatalkan</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="2" class="text-center py-8 text-sm text-gray-500">Belum ada aktivitas reservasi.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
